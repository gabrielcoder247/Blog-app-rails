...
RSpec.feature 'Post #Index',
  ...
  background do
    ...
    @mark_post = Post.create(title: 'mark post title',
                             text: 'mark post text', author_id: @first_user.id, likes_counter: 0)
  ...
  it 'sees the users profile picture' do
    ...
    expect(page.body).to include('<li class="padding-20-px background-color-aliceblue">')
  end
  ...
  it 'sees the first post comment' do
    @first_comment = Comment.create(text: 'marks post first comment', author_id: @first_user.id, post: @mark_post)
    visit user_posts_path(@first_user.id)
  ...
  it 'sees the post comments count' do
    ...
    expect(page).to have_content(
      "Comments: #{@mark_post.CommentsCounter}"
    )
  end
  ...
  it 'sees pagination button if there are more than 6 posts' do
    visit user_posts_path(@first_user.id)
    expect(page.has_button?('Create post')).to be true
  end
